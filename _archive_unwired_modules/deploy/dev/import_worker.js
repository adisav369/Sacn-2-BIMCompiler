...
self.onmessage = async function(e) {
  ...
  try {
    ...
    // Mirrors backend scripts/extractIFC2DB.js:448-517, PLUS the dual-direction
    // IfcRelAssignsToProduct loop (the Hospital fix — Bonsai writes element⇄task
    // links as RelatingProduct=element, RelatedObjects=tasks). ifcApi + modelID
    // are still valid here (before CloseModel below).
    // T1b (§5.2): widened capture — CPM dates, float, critical, WBS hierarchy, calendar.
    ...
    // WBS hierarchy (§5.2): IfcRelNests maps parent task → child tasks. We capture
    // wbs_parent (parent GUID) and is_summary (a task that HAS children). Summary-vs-leaf
    // is NOT reliable from PredefinedType (W-VOCAB) — "summary = has children" is the rule.
      ...
      for (var tki = 0; tki < taskIds.size(); tki++) {
        try {
          ...
          tasks.push({
            ...
            // EXTRACT VERBATIM — durations/floats are ISO-8601 ("P15D"/"P0D"); keep raw, never
            // re-derive (PRIME RULE). parseFloat("P15D")=NaN, so the T1 numeric path lost them.
    ...
    // Resolve WBS parent GUID + summary flag now that both maps exist.
    ...
    // Work calendar (§5.2): thin {name, recurrence_type, raw} carrier. Hours live in the
    // WorkingTime name ("9 - 5"); do NOT invent hours_per_day. Calendar conversion is DEFERRED
    // (user 2026-05-30, round-the-clock for now) — we capture the carrier so it's there later.
    try {
    ...
    if (schedules.length || tasks.length) {
      ...
      // T1b W-VOCAB witness — proves the widened fields populated at the §5.2 evidence rates.
    ...
    const result = {
      ...
      // 4D_CAPTURE_AND_FALLBACK.md T1 — native IFC 4D schedule (W-CAPTURE)
      schedules: schedules,
      tasks: tasks,
