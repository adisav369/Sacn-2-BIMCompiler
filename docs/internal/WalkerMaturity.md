...
| **ELEC** | SEED-TRUNK (entry→3D trunk) | **L4 (LIVE)** | GENERATED+seed | **W-SEED-TRUNK** / **-DEFAULT** / **-CORRIDOR-TRUNK** / **-RISER-TRUNK** 6/6 each + **W-SEED-TRUNK-RENDER 6/6** (modeller render GATE) | seed→corridor trunk (around walls/through doors, 10→0 crossings) now 3D: per-storey trunks joined by RISERS at REAL IfcStairs (Duplex: 1 seed feeds 2 floors via 2 real stairs, 55 fixtures trace to it; multi-riser lifts L2 reach 13→25, 8 REFUSED). **RENDER GATED** (W-SEED-TRUNK-RENDER, `modeller/tests/witness_seed_trunk_render.js`, headless swiftshader): `__seedTrunkProbe` scene-graph census proves the rendered LineSegments == `planTrunk` net (vertices==2×data segs, risers==climbing-risers, firstVertex drift 2.4e-9m, canvas litPct 64%) — eyeball gap CLOSED. Gate caught a real bug: `_renderSeedTrunk`/`seedTrunk` guarded on `net.refused` truthy, but on a SUCCESS net `refused` is the integer COUNT of unreachable fixtures → any building with a refused fixture rendered an EMPTY trunk (Duplex ELEC refuses 112/267); fixed to `=== true` (boolean refusal only). |
...
- **SEED-TRUNK L1→L2→L3 (the human-in-the-loop seed, user idea 2026-06-30):** `W-SEED-TRUNK` proved the mechanism — an
  ...
  floors all trace to the single seed; vertical only at the stair XY). MULTI-RISER DONE (RS6): each upper fixture fed by its NEAREST REACHABLE real stair → Level-2 reach
  13→25 vs one riser (+12), wall-cross 0+0, 8 honestly refused (no stair-reachable path). All 55 served fixtures on 2
  floors trace to the single seed. NEXT: (1) promote the spike into an engine fn (disc_walker.seedTrunk) + modeller
  render; (2) a held-out check.
