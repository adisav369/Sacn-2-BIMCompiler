...
Built on [iDempiere](https://idempiere.org/) ERP conventions and [SQLite](https://www.sqlite.org/) — upgraded to a browser-native Three.js viewer (S200+). From Kuala Lumpur, Malaysia — where BIM is [mandated](StrategicIndustryPositioning.md) for all projects >= RM10M from July 2025. It is the Creator, [Redhuan D. Oon](mailto:red1org@gmail.com)'s gift to the world.
...
-   **33 Buildings Compiled**

    ---

    33 extracted from real [IFC](https://www.buildingsmart.org/standards/bsi-standards/industry-foundation-classes/) files.
...
-   **77 Verbs, 2,475 Products (BIM Compiler era)**
...
33 buildings compiled — from a single-storey house to a multi-storey airport terminal. 19 pass all 6 mathematical gates. Once a stone is certified, any new building composed from its proven grammar inherits the proof — no new reference needed.
...
## **Design in Browser, Compile from BOM**
...
The compiler powers a live GUI in the browser via the [BIM OOTB viewer](https://red1oon.github.io/bim-ootb/) and the [Doc Pill design interface](NEW_FROM_REFERENCE.md). Create a building from a single click, edit room dimensions with sliders, switch jurisdictions, save/recall design versions, and promote a design to a construction-ready work order.

The viewer provides BOM trees, geometry inspection, validation, and colour-coded discipline breakdown. [DocAction](DocAction_SRS.md) lifecycle buttons (Draft → Approve → Complete → Promote) drive the design through ERP-standard approval stages.

552+ commits since S200. What the user sees IS what the compiler produces, deterministically.
...
**Browser viewer (no install):** Open [red1oon.github.io/bim-ootb](https://red1oon.github.io/bim-ootb/) — drop any IFC file. No Java, no Maven, no prerequisites.

**BIM Compiler (Java backend — historical):**
