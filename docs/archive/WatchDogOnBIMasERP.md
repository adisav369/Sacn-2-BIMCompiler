...
> (all in `library/{PREFIX}_BOM.db`). `FixturePlacer` + `FurnitureTypeResolver` **deleted**.
...
loaded from `m_bom` ({PREFIX}_BOM.db). This worked but could silently fail if a Revit family string happened
...
seeded in `m_bom` ({PREFIX}_BOM.db) with `bom_type IN ('UNIT','FLOOR')`. The 5-tier CHECK constraint
...
Gate: `SELECT bom_id FROM m_bom WHERE bom_id NOT IN (SELECT DISTINCT bom_id FROM m_bom_line WHERE is_active=1)` before any BOM milestone (run against {PREFIX}_BOM.db).
...
WHERE is_active = 1
  ...
  AND family_ref NOT IN (SELECT bom_id FROM m_bom);  -- {PREFIX}_BOM.db
```
...
| Column | Table | Change |
|--------|-------|--------|
| `bom_category TEXT DEFAULT NULL` | `m_bom` ({PREFIX}_BOM.db) | NULL = global; 'SH'/'DX'/'TB'/'MY'/'TL' = scoped |
...
The connection to BOM is clean: a fabricated mesh leaf node uses `m_attribute` ({PREFIX}_BOM.db)
