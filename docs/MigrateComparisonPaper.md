...
> **Exhaustive coverage map → [ERP Coverage Matrix](ERP_COVERAGE_MATRIX.md).** Every rule/process surface in *both* homes (Java code + the AD as data) enumerated, each count from a real `ad_full.db` query: **0 covered / 37 partial / 3 gap** of 40 surfaces (the interpreter-coverage ladder, now closed for every seed-data surface) **+ a second *equivalence* axis: 16 surfaces now fold to real GardenWorld `fact_acct` to the cent (`maxDiff=0c`)** — the whole order→ship→invoice→match→pay→allocate trade loop, the movement→on-hand→replenish inventory loop, inter-org `M_Movement` + `GL_Journal`, the `reverseCorrect`/void family, and the `M_Movement`/`M_MatchInv` folds in **both** accounting schemas (USD + EUR) — **plus 3 rule-consistent** (enacted docs with no seed oracle: BOM backflush, `MProduction`, `MInventory` — verified against the proven qty/cost rules + balance + falsifier). ([ERP_MODEL_ARCHETYPE.md](ERP_MODEL_ARCHETYPE.md) is the M-class denominator: MOrder archetype + ~25 deltas — **the deepest deltas now fold `maxDiff=0c`**.) The buckets below are measured, not asserted.
