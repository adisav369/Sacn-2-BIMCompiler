...
<p style="margin:16px 0;font-size:15px"><a href="../bench_suite.html"><b>Benchmark Comparison</b> — the engine vs iDempiere, running live in your browser &nbsp;⏱️&nbsp; ↗</a> &nbsp;·&nbsp; <span style="opacity:.7">both sides measured from real source: 28K JS vs 1.4M Java LOC (51×), 1.7 vs 43 MB (26×) — re-counted on every deploy, so the number tracks the code, not a frozen claim.</span></p>
...
1. **SAP S/4HANA fold** — BLOCKED. `build/erp/sap_fold.log` says `§SAP-FOLD NOT-RUN (skeleton ready;
   ...
   *partly mock, partly not-run* — never present S/4HANA as proven. The *plan* is published and the
   target fixed: the **[ACDOCA Fold Plan](ACDOCA_FOLD_PLAN.md)** documents how S/4HANA's Universal
