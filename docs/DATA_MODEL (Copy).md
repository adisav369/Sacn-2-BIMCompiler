...
### BOM.db Population — Source of Truth

BOM.db is regenerated from scripts, never hand-edited. Two pipelines:

- **AD Metadata:** 14 scripts at `scripts/create_ad_*.py` — idempotent (DELETE + INSERT), run-order dependent.
- **BOM Content:** `scripts/RosettaStoneToBOM.py` — sole source of truth for m_bom + m_bom_line.
- **Legacy:** `migration/archive/*.sql` — historical only, not executed.
