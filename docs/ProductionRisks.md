...
- **🔬 Discovery spike (S2/S3, MEASURED 2026-06-24 — `scripts/spike_s1_discovery.js`, log `logs/spike_s1_discovery_20260624_101214.log`):** `§SPIKE-B1 device=proxy(node18-v8/wasm,NOT-a-phone) elements=63181 peakHeapMB=239.8 dbFileMB=266.8 geomBytesMB=232.5 projectedMobileResidentMB=499.3 budgetMB=200 crashed=Y(projected) ttiMs=132 geoms=19928`. **Verdict — the ceiling is breached by the largest committed building, BEFORE the renderer even decodes.** Hospital_3 (63,181 elements) is a **266.8 MB DB file** holding **232.5 MB of raw vertex/face BLOB**; loaded into the in-memory sql.js substrate (the viewer's own path) it already **overshoots the 200 MB mobile ceiling on the DB alone**, and once three.js decodes the BLOBs to typed arrays the projected resident set is **~499 MB ≈ 2.5× budget** — squarely in iOS-Safari tab-kill territory. **This makes §13 gravity-sharding URGENT, not design-only:** the headline building cannot be shipped whole to a mid-tier phone. ⚠ Number is a **node V8/WASM proxy, NOT a real device** — the device-tier soak (test-plan row 1) is still 🔴 and must confirm on a real Android/iPhone; but the proxy already disproves "it fits." (122K-element buildings referenced in §B1 prose would be ~2× worse again.)
