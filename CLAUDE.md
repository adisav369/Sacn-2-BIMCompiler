...
- **OCI MIME Rule:** EVERY `oci os object put` MUST include `--content-type`. OCI does NOT infer MIME from file extension. `.js` → `"application/javascript"`, `.html` → `"text/html"`, `.json` → `"application/json"`, `.png` → `"image/png"`. Omitting this causes browser MIME mismatch block (`X-Content-Type-Options: nosniff`) and silent script failures.
