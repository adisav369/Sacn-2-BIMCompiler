...
function setupStreaming(A) {
  ...
  A._flushInstanced = function() {
    ...
    // Catches routing bugs (like S280b threshold change) immediately via console
    if (A.streamIdx >= A.streamQueue.length) {
      ...
      // On mobile, merged elements have no per-GUID meta — only check desktop
      ...
      // guidMap must have an entry for every registered element
      ...
      // Routing invariant: InstancedMesh must only contain 2+ instance hashes
