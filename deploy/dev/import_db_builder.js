...
function buildImportDBs(SQL, data) {
  ...
  // IDENTICAL DDL to scripts/extractIFC2DB.js:538-541 (Single-DB standard).
  // Tables always created (empty if IFC carries no programme) so consumers
  // (Time Machine) can probe a stable schema.
  db.run('CREATE TABLE IF NOT EXISTS schedules (schedule_id TEXT PRIMARY KEY, name TEXT, status TEXT, created_date TEXT)');
  // T1b widened schema (§5.2, W-VOCAB-backed): adds CPM dates (early/late), float, is_critical,
  // WBS (wbs_parent + is_summary), predefined_type, and a nullable resource (OUR template concept,
  // captured=null). schedule_* / float fields are RAW ISO-8601 strings (verbatim — never re-derived).
  ...
  var _nSched = 0, _nTask = 0, _nSeq = 0, _nTE = 0;
