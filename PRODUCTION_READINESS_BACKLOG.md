...
> Updated 2026-09-10: the `IfcRoof (plane)` catalog gap blocking Building A's multi-storey
> compile is **FIXED** (commit `868d349dc`) — a real `component_definitions`/
> `component_geometries` entry, sized to the real median of 1,144 actually-measured roof
> segments, verified by reproducing the exact failure and confirming it's gone. Compile now
> runs much further (all 175 elements' real geometry written, every GEO forensic proof green)
> before hitting a NEW, separate, not-yet-investigated finding — **A11**, an element-count
> mismatch (`expected=2919` vs `actual=175`) that doesn't match the mild ~1.2x factorization
> ratio measured at BOM-build time. Full compile on the multi-storey building is still not
> reached; one blocker down, a different one found behind it.
>
...
| A4 | Multi-storey detection (currently hardcoded to 1 storey per building) | **re-scoped 2026-09-07, write-side + real-data BOM build DONE 2026-09-08, catalog gap FIXED 2026-09-10** | The realistic first step (formalize Building A's 2 real floors into one real multi-storey `write_reference_db` output) is implemented and run end to end against REAL DeKH Building A data: `PIPELINE auto-discovered 2 spatial containers`, built 2 real per-storey BOMs, all QA gates passed. The `IfcRoof (plane)` catalog gap that blocked full compile is now fixed (commit `868d349dc`, full detail in `SEQUENCED SESSION PLAN` §2) — compile proceeds past it, but a NEW, separate finding (**A11**) now blocks compile-to-completion. Auto-detecting floors from one continuous scan (the harder sub-problem) remains unscoped. Full detail in `SEQUENCED SESSION PLAN` §2. |
...
- **(a) DONE 2026-09-08 — write-side only, not yet run against real Building A data.**
  ...
  **Run end-to-end against REAL Building A point-cloud data, same session** (the real `.laz`
  files turned out to be available at `C:\DeKH\Buildings\` — an earlier check this same
  session wrongly concluded otherwise, corrected once found; see the `reference_dekh_data_
  location` memory). Both floors (507M / 621M raw points) segmented for real, verifying A10's
  fix on a second scene along the way (zero pathological mega-merges, max group size 10 on
  both floors). `--stage combine-storeys` produced a genuine 2-storey reference DB (Level 1
  elevation -3.56m/1,424 confident elements, Level 2 elevation 0.58m/1,495) — verified
  directly (1 building, 2 storeys, zero orphan FKs, zero guid collisions). Ran through the
  actual unmodified Java chain with scratch `--comp-db`/`--erp-db` isolation (real tracked
  library confirmed byte-identical before/after via sha256): `--populate --classify`
  succeeded — `PIPELINE auto-discovered 2 spatial containers from extraction: [Level 1,
  Level 2]`, 2 real per-storey BOMs built (1,421 / 1,485 lines, matching the Python-side
  element counts exactly), **all `BomValidator` QA checks passed**, including the per-storey
  AABB containment checks. Compile itself (temporarily gate-scoped for this one-off
  verification, reverted immediately after) got genuinely far — BOM walk, discipline
  routing, MEP routes, `C_Order` created, 2,914 real `C_OrderLine` rows written — before
  failing on `MetadataMissingException: No geometry for IfcRoof element_ref=IfcRoof (plane)
  ... [NO FALLBACK]`. **This is a real but UNRELATED, pre-existing gap**:
  `component_library.db` has no generic geometry entry for the point-cloud pipeline's
  `"IfcRoof (plane)"` product family — a catalog-completeness issue, correctly refusing to
  invent geometry per the Prime Rule, not a defect in the multi-storey write or A10.

  **The `IfcRoof (plane)` catalog gap — FIXED 2026-09-10 (commit `868d349dc`).** Added one
  real `component_definitions` + `component_geometries` row: a simple box, same binary
  convention (float32/int32 little-endian, 8 vertices/12 triangles) as a real, already-working
  catalog entry (confirmed by reading that entry's own blob bytes directly), sized to the REAL
  median of the 1,144 actually-measured `IfcRoof`-classified segments in this exact building —
  5.819×4.162×1.740m — never an invented placeholder. `component_types` already had a real
  `('IfcRoof','ROOF','ARC')` row (id=22) waiting unused — the schema already anticipated this
  entry, it was just never populated. Verified by reproducing the actual failure and fixing
  it, not assumed: built and tested the fix in a scratch copy first, confirmed byte-identical
  before applying it permanently, then used the same swap→run→restore→verify pattern as D4/D1
  (real library confirmed byte-identical before/after via sha256) to prove it against the real
  tracked file. Purely additive — row counts +1/+1, verified directly, no existing row touched.

  **What this unblocked, and what it surfaced next:** the `MetadataMissingException` is gone —
  compile now proceeds through writing all 175 real elements' geometry and every GEO forensic
  proof (175/175 OK, 0 FAIL). But it then fails a DIFFERENT, previously-hidden assertion —
  **new item A11** below. **The multi-storey structure itself remains proven correct through
  BOM build**; full compile-to-completion is blocked by this new, separate finding, not by
  anything this fix touched. (b) [auto-detecting floors from one continuous scan] stays
  logged, not started, pending real client-scan-practice information.
