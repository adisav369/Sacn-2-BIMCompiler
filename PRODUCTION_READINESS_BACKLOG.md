...
| A4 | Multi-storey detection (currently hardcoded to 1 storey per building) | **re-scoped 2026-09-07, write-side + real-data BOM build DONE 2026-09-08** | The realistic first step (formalize Building A's 2 real floors into one real multi-storey `write_reference_db` output) is implemented and run end to end against REAL DeKH Building A data (both floors, 507M/621M raw points): `PIPELINE auto-discovered 2 spatial containers`, built 2 real per-storey BOMs, all QA gates passed, 2,914 real `C_OrderLine` rows written. Full compile blocked on an unrelated, pre-existing `component_library.db` gap (no generic geometry for the `IfcRoof (plane)` family) — not a defect in this work. Auto-detecting floors from one continuous scan (the harder sub-problem) remains unscoped. Full detail in `SEQUENCED SESSION PLAN` §2. |
...
- **(a) DONE 2026-09-08 — write-side only, not yet run against real Building A data.**
  ...
  **Run end-to-end against REAL Building A point-cloud data, same session** (the real `.laz`
  ...
  invent geometry per the Prime Rule, not a defect in the multi-storey write or A10. Not
  chased further this session — needs a real catalog entry, its own small follow-on task.
  **The multi-storey structure itself is proven correct end-to-end through BOM build**; only
  the final geometry-instancing step for one specific product family remains blocked. (b)
  stays logged, not started, pending real client-scan-practice information.
