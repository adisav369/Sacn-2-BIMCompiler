 ...
 * check (relative offsets) never saw it. A SECOND bug: the drop yaw was threaded through the BOM recursion's
 * cumRot, where MIRROR (= rot=π) swallows it — a rotated mirror-building drop scattered 45m. THE FIX:
 * Library.dropLeaves expands the CANONICAL building (rot=0, the proven Java path untouched) and applies the drop
 * yaw as an EXTERNAL RIGID rotation about the cursor (PlacementCollectorVisitor.java:347-374 has no drop yaw —
 * cumRot/cumMirror are the building's INTRINSIC transforms; the drop rotation belongs outside the recursion).
 *
 * CLAIMS (read the §-log; exit 1 on any failure; NON-INVENT — every number computed from the catalog):
 *   G1  the OLD centring (declared-aabb half) is provably off-cursor by metres (documents the bug the fix removes).
 *   G2  the SHIPPED Library exposes dropLeaves + footprintAABB.
 *   G3a the BASIC drop (yaw=0): EVERY assembly's footprint centre lands on the cursor to <1mm.
 *   G3b ROTATED drops (yaw 45/90/180/270), EVERY assembly incl the 2 MIRROR buildings: rotate-about-cursor <1mm.
 */
'use strict';
const fs = require('fs');
...
(async function main() {
  ...
  if (typeof L.dropLeaves !== 'function' || typeof L.footprintAABB !== 'function') {
    fail('G2 shipped Library missing dropLeaves/footprintAABB — fix not implemented');
    log('§W-BOM-DROP-CENTER FAIL: ' + fails.join('; ')); process.exit(1);
  }
  log('§W-BOM-DROP-CENTER G2 dropLeaves + footprintAABB present in shipped module');

  // does this assembly's subtree carry a MIRROR anywhere? (the case that used to scatter 45m under rotation)
  ...
  // The yaw invariant is "the drop rigidly ROTATES the canonical building ABOUT the cursor". Test it the hard way:
  // drop at yaw, UN-rotate the landed leaves about the cursor by −yaw, and the recovered footprint centre must sit
  // on the cursor (and equal the yaw=0 footprint). NOT "the rotated cluster's axis-aligned AABB centre == cursor"
  // (an AABB reshapes under rotation). Holds for ANY assembly — incl. MIRROR — iff the drop yaw is applied as an
  // external rigid transform on the canonical (Java-faithful) expansion, which is what dropLeaves does.
  const offFor = (id, yaw) => {
    const leaves = L.dropLeaves(id, CURSOR.x, CURSOR.y, yaw, 0);
  ...
  // G3b — ROTATED drops: NOW ASSERTED for EVERY assembly at every yaw, including the 2 MIRROR buildings (which
  // used to scatter 45m). The drop yaw is an external rigid rotation about the cursor; the Java-faithful canonical
  // expansion (expandAssembly @ rot=0) is untouched. Must hold to <1mm.
  const YAWS = [45, 90, 180, 270];
  const mirN = droppable.filter(a => hasMirror(a.id)).length;
  let rWorst = 0, rWorstId = '', rChecks = 0;
  for (const a of droppable) for (const yaw of YAWS) {
    const off = offFor(a.id, yaw); rChecks++;
    if (off > rWorst) { rWorst = off; rWorstId = a.id + '@' + yaw + '°' + (hasMirror(a.id) ? ' [MIR]' : ''); }
    if (off > TOL) fail('G3b ' + a.id + ' yaw=' + yaw + '° not rotate-about-cursor: off ' + (off * 1000).toFixed(1) + 'mm');
  }
  log('§W-BOM-DROP-CENTER G3b ROTATED drops (' + droppable.length + ' asm incl ' + mirN + ' MIRROR × ' +
      YAWS.length + ' yaws = ' + rChecks + ' checks): worst off-cursor=' + (rWorst * 1000).toFixed(2) + 'mm on ' + rWorstId);

  if (fails.length) {
    log('\n§W-BOM-DROP-CENTER FAIL (' + fails.length + '):'); fails.slice(0, 12).forEach(f => log('  - ' + f));
    process.exit(1);
  }
  log('\n§W-BOM-DROP-CENTER PASS — every one of ' + droppable.length + ' assemblies drops with its footprint CENTRE ' +
      'on the cursor to <1mm, at yaw 0/45/90/180/270, INCLUDING the MIRROR buildings (was up to ' + oldWorst.toFixed(1) +
      'm off un-rotated / 45m rotated). Fix: drop yaw = a RIGID rotation about the cursor over the canonical ' +
      '(Java-faithful, rot=0) expansion — expandAssembly UNTOUCHED (W-MODELLER-DROP host==oracle 0.000mm).');
  process.exit(0);
})();
