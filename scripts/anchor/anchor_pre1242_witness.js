...
// §S42 ANCHOR — the pre-#1242 engine (0fe8eb2^, 2026-08-07: sort order + physics as a clock, no
// geometry DAG, no blob, no cycles). Loaded as a SECOND module; it assigns global.ScheduleGate on
// load, so today's is saved and restored around the require. Only the SCHEDULER differs — the
...
(async () => {
  ...
  for (const bld of BUILDINGS) {
    ...
    // §S42 — same elements, same rates, same crews, same judge; pre-#1242 scheduler.
    let anchorFloat = 'ERR', anchorSpan = 'ERR', anchorMidair = 'ERR', anchorOrphan = 'ERR';
    try {
      const copy = geoEls.map(e => Object.assign({}, e));
      const qq = console.log; console.log = () => {};
      let oldSched;
      try { oldSched = SG_PRE1242.computeSchedule(copy, 0, 1, maxCrews); } finally { console.log = qq; }
      const mOld = {};
      geoEls.forEach(e => { const r = oldSched[e.guid]; if (r) mOld[e.guid] = { start: r.start, end: r.end }; });
      const q2 = console.log; console.log = () => {};
      try { anchorFloat = ScheduleGate.auditFloating(geoEls, mOld); } finally { console.log = q2; }
      let mx = 0; Object.keys(oldSched).forEach(g => { if (oldSched[g].end > mx) mx = oldSched[g].end; });
      anchorSpan = (mx / 86400000).toFixed(1);
      // strict midair on the SAME pre-#1242 times, using this witness's own census (W-MZ-2's judge)
      const oldItems = geoEls.filter(e => oldSched[e.guid]).map(e => ({ guid: e.guid,
        s: oldSched[e.guid].start, e: oldSched[e.guid].end, bz: e.base_z, tz: e.top_z,
        x0: e.x0, x1: e.x1, y0: e.y0, y1: e.y1, cls: e.cls, seq: e.seq, phase: e.phase,
        storey: e.storey, resource: e.resource }));
      const oc = census(oldItems);
      anchorMidair = oc.midair; anchorOrphan = oc.orphan;
    } catch (e) { anchorFloat = 'ERR:' + (e && e.message || e); }
    let todayPreSpan = 0; items.forEach(it => { if (it.e > todayPreSpan) todayPreSpan = it.e; });
    console.log('§S42_ANCHOR ' + bld + ' pre1242Float=' + anchorFloat + ' todayPreCpmFloat=' + floatPre +
      ' pre1242MakespanDays=' + anchorSpan + ' todayPreCpmMakespanDays=' + (todayPreSpan / 86400000).toFixed(1) +
      ' pre1242Midair=' + anchorMidair + ' pre1242Orphan=' + anchorOrphan + ' — same elements, same DB, same judge (today auditFloating + this witness census)');
    ...
    let postSpan = 0; items.forEach(it => { if (it.e > postSpan) postSpan = it.e; });
    console.log('§S42_TODAY ' + bld + ' postCpmFloat=' + floatPost + ' postCpmMakespanDays=' + (postSpan / 86400000).toFixed(1));
