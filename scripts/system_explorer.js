...
'// the TIDY MATRIX target (gx,gy): the balls SORT THEMSELVES to minimise line-bunching \\u2014 snap to a grid by',
'// organic position, then run BARYCENTER sweeps (each node eases to the avg grid-cell of its neighbours) so',
'// connected bubbles cluster and edges shorten/uncross. Grid is sized larger than the blob so balls space out.',
'(function computeTidy(){var adj={};E.forEach(function(e){(adj[e.from]=adj[e.from]||[]).push(e.to);(adj[e.to]=adj[e.to]||[]).push(e.from);});',
' var nodes=N.slice(),C=Math.ceil(Math.sqrt(nodes.length)),rows=Math.ceil(nodes.length/C);',
' function assign(){var by=nodes.slice().sort(function(a,b){return a._oy-b._oy||a._ox-b._ox;});for(var r=0;r<rows;r++){by.slice(r*C,(r+1)*C).sort(function(a,b){return a._ox-b._ox;}).forEach(function(n,c){n._col=c;n._row=r;});}}',
' nodes.forEach(function(n){n._ox=n.x;n._oy=n.y;});assign();',
' for(var s=0;s<12;s++){nodes.forEach(function(n){var nb=adj[n.id]||[],sx=0,sy=0,k=0;nb.forEach(function(m){var mn=N[idx[m]];if(mn&&mn._col!=null){sx+=mn._col;sy+=mn._row;k++;}});n._ox=k?sx/k:n._col;n._oy=k?sy/k:n._row;});assign();}',
' var xs=N.map(function(n){return n.x;}),ys=N.map(function(n){return n.y;}),spanX=Math.max.apply(null,xs)-Math.min.apply(null,xs),spanY=Math.max.apply(null,ys)-Math.min.apply(null,ys);',
' var cellX=Math.max(82,1.35*spanX/Math.max(1,C-1)),cellY=Math.max(76,1.2*spanY/Math.max(1,rows-1));',
' nodes.forEach(function(n){n.gx=CX+(n._col-(C-1)/2)*cellX;n.gy=CY+(n._row-(rows-1)/2)*cellY;});})();',
...
'function project(n){var ax=n.x-CX,ay=n.y-CY,az=(n.z||0);var cY=Math.cos(yaw),sY=Math.sin(yaw),cP=Math.cos(pitch),sP=Math.sin(pitch);var X1=ax*cY+az*sY,Z1=-ax*sY+az*cY;var Y2=ay*cP-Z1*sP,Z2=ay*sP+Z1*cP;var rsx=CX+X1,rsy=CY+Y2;var t=orbitAmt();t=t*t*(3-2*t);var gx=(n.gx==null?rsx:n.gx),gy=(n.gy==null?rsy:n.gy);n.sx=rsx+(gx-rsx)*t;n.sy=rsy+(gy-rsy)*t;n.depth=Z2*(1-t);}',
