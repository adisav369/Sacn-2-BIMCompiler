...
'  <div class="ctl"><button id="traceBtn">▸ Trace a record</button><button id="muteBtn" title="mute / unmute the interface sounds">&#128266;</button><button id="resetBtn">⤢ Reset view</button><button id="panelToggle" title="hide / show the info panel">⟩</button></div>\n' +
'  <div id="strip"></div>\n' +
...
'// W-ARRANGE: leaning the trackball MORPHS the organic blob \\u2192 a tidy BFS-ordered MATRIX (grid), so the',
'// bubbles space out and the edges un-bunch. orbitAmt drives the morph t; at rest t=0 \\u2192 identity preserved.',
...
'// the 3D LAYERED CUBE: split bubbles into their spine ROLE layers (reference back / doc-flow mid / settlement',
'// front), lay each layer out as its OWN self-sorting grid (barycentre sweeps align connected bubbles across',
'// layers so cross-layer flow edges stay short), and stack the layers in depth (gz). Lean = the cube; rotate to read it.',
'(function computeTidy(){var adj={};E.forEach(function(e){(adj[e.from]=adj[e.from]||[]).push(e.to);(adj[e.to]=adj[e.to]||[]).push(e.from);});',
' var ZC=270,LZ=[-ZC,0,ZC];function layerOf(n){return n.settlement?2:(n.kind==="master"?0:1);}',
' var groups=[[],[],[]];N.forEach(function(n){groups[layerOf(n)].push(n);});',
' var xs=N.map(function(n){return n.x;}),ys=N.map(function(n){return n.y;});',
' var spanX=Math.max.apply(null,xs)-Math.min.apply(null,xs);',
' var maxC=Math.max.apply(null,groups.map(function(g){return Math.ceil(Math.sqrt(g.length||1));}));',
' var cell=Math.max(92,1.25*spanX/Math.max(1,maxC-1));',
' groups.forEach(function(g){if(!g.length)return;g.C=Math.ceil(Math.sqrt(g.length));g.rows=Math.ceil(g.length/g.C);',
'  g._assign=function(){var by=g.slice().sort(function(a,b){return a._oy-b._oy||a._ox-b._ox;});for(var r=0;r<g.rows;r++){by.slice(r*g.C,(r+1)*g.C).sort(function(a,b){return a._ox-b._ox;}).forEach(function(n,c){n._col=c;n._row=r;});}};',
'  g.forEach(function(n){n._ox=n.x;n._oy=n.y;});g._assign();});',
' for(var s=0;s<14;s++){N.forEach(function(n){var nb=adj[n.id]||[],sx=0,sy=0,k=0;nb.forEach(function(m){var mn=N[idx[m]];if(mn&&mn._col!=null){sx+=mn._col;sy+=mn._row;k++;}});if(k){n._ox=sx/k;n._oy=sy/k;}else{n._ox=n._col;n._oy=n._row;}});groups.forEach(function(g){if(g._assign)g._assign();});}',
' groups.forEach(function(g,li){if(!g.length)return;g.forEach(function(n){n.gx=CX+(n._col-(g.C-1)/2)*cell;n.gy=CY+(n._row-(g.rows-1)/2)*cell;n.gz=LZ[li];});});})();',
...
'function project(n){var t=orbitAmt();t=t*t*(3-2*t);var ex=n.x+((n.gx==null?n.x:n.gx)-n.x)*t,ey=n.y+((n.gy==null?n.y:n.gy)-n.y)*t,ez=(n.z||0)+((n.gz==null?(n.z||0):n.gz)-(n.z||0))*t;var ax=ex-CX,ay=ey-CY,az=ez;var cY=Math.cos(yaw),sY=Math.sin(yaw),cP=Math.cos(pitch),sP=Math.sin(pitch);var X1=ax*cY+az*sY,Z1=-ax*sY+az*cY,Y2=ay*cP-Z1*sP,Z2=ay*sP+Z1*cP;n.sx=CX+X1;n.sy=CY+Y2;n.depth=Z2;}',
...
' E.forEach(function(e){if(!show[e.kind])return;var a=N[idx[e.from]],b=N[idx[e.to]];if(!a||!b)return;var lo=litEdge(e),ef=fo&&edgeFocus(e),op,sw,col=lo?"#ffd479":COLOR[e.kind];if(traceMode){op=lo?0.95:0.04;sw=lo?3:0.8;}else if(fo){op=ef?0.85:0.035;sw=ef?2:0.6;if(ef)col="#ffd479";}else{op=e.kind==="reference"?0.18*(1-0.92*oa):0.5;sw=e.kind==="reference"?1:1.6;}  /* fade the bunched reference web as you organise → only the clean spine remains */var ln=el("line",{x1:a.sx,y1:a.sy,x2:b.sx,y2:b.sy,stroke:col,"stroke-opacity":op,"stroke-width":sw});if(lo)ln.setAttribute("class","litedge");vp.appendChild(ln);});',
...
'function setOrbit(y,p){yaw=Math.max(-1.25,Math.min(1.25,y));pitch=Math.max(-0.95,Math.min(0.95,p));window.__yaw=yaw;window.__pitch=pitch;updateBall();draw();}  /* clamp so leaning further can\\u2019t over-rotate the cube past a readable angle */',
