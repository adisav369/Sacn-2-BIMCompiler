...
'var yaw=0,pitch=0,SP=1,SPREADK=0.8;window.__yaw=0;window.__pitch=0;',
'// SP = declutter spread: 1 at rest, grows with orbit so leaning the trackball SPACES the bubbles apart',
'// (its main job — arrange the view; further = more organised). At rest SP=1 → identity preserved.',
...
'function project(n){var ax=(n.x-CX)*SP,ay=(n.y-CY)*SP,az=(n.z||0)*SP;var cY=Math.cos(yaw),sY=Math.sin(yaw),cP=Math.cos(pitch),sP=Math.sin(pitch);var X1=ax*cY+az*sY,Z1=-ax*sY+az*cY;var Y2=ay*cP-Z1*sP,Z2=ay*sP+Z1*cP;n.sx=CX+X1;n.sy=CY+Y2;n.depth=Z2;}',
'// inverse: recover a node\\u2019s plane (x,y) from a desired screen point + its fixed z (keeps bubble-drag working under orbit).',
'function unproject(sx,sy,az){var cY=Math.cos(yaw),sY=Math.sin(yaw),cP=Math.cos(pitch),sP=Math.sin(pitch);if(Math.abs(cY)<0.15||Math.abs(cP)<0.15)return null;var azS=(az||0)*SP;var ax=((sx-CX)-azS*sY)/cY;var Z1=-ax*sY+azS*cY;var ay=((sy-CY)+Z1*sP)/cP;return {x:CX+ax/SP,y:CY+ay/SP};}',
...
'function draw(){SP=1+orbitAmt()*SPREADK;N.forEach(project);while(vp.firstChild)vp.removeChild(vp.firstChild);var oa=orbitAmt(),fo=focusId&&!traceMode;',
