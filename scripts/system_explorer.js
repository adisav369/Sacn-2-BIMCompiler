...
'// the TIDY MATRIX target (gx,gy) per node: BFS-order from the busiest hubs so CONNECTED bubbles land in',
'// adjacent grid cells \\u2192 edges stay short + un-bunched; grid is sized larger than the blob so balls space out.',
'(function computeTidy(){var deg={},adj={};E.forEach(function(e){deg[e.from]=(deg[e.from]||0)+1;deg[e.to]=(deg[e.to]||0)+1;(adj[e.from]=adj[e.from]||[]).push(e.to);(adj[e.to]=adj[e.to]||[]).push(e.from);});',
' var order=[],seen={},starts=N.slice().sort(function(a,b){return (deg[b.id]||0)-(deg[a.id]||0);});',
' starts.forEach(function(s){if(seen[s.id])return;var q=[s.id];seen[s.id]=1;while(q.length){var id=q.shift();order.push(id);(adj[id]||[]).slice().sort(function(a,b){return (deg[b]||0)-(deg[a]||0);}).forEach(function(m){if(!seen[m]){seen[m]=1;q.push(m);}});}});',
...
' var C=Math.ceil(Math.sqrt(order.length)),rows=Math.ceil(order.length/C);',
' var cellX=Math.max(82,1.35*spanX/Math.max(1,C-1)),cellY=Math.max(76,1.2*spanY/Math.max(1,rows-1));',
' order.forEach(function(id,i){var col=i%C,row=Math.floor(i/C),n=N[idx[id]];n.gx=CX+(col-(C-1)/2)*cellX;n.gy=CY+(row-(rows-1)/2)*cellY;});})();',
