#!/usr/bin/env node
// §S43 HULL PROBE — copy into bim-ootb/viewer/tests/ and run from there (relative requires).
...
(async () => {
  ...
  for (const bld of BUILDINGS) {
    ...
    // ── §S43 HULL (B1) ────────────────────────────────────────────────────────────────────────
    // time_machine.js:6074 groups kernel_ops by task identity first, `storey|phase` as the
    // fallback — and the shipped task tables hold 0 rows on every fleet building (§S26.13), so
    // storey|phase IS the live grouping. Each bar's span is then _tukeyBound(starts,true) ..
    // _tukeyBound(ends,false) (§GANTT_MINI_TRIM). Measured here: the RAW hull the elements
    // actually describe, the TRIMMED bar that ships, and how much of the raw hull is owed to ONE
    // element — the number that says whether fixing the grain is worth it.
    ...
    const gmap = {};
    items.forEach(it => {
      const k = (it.storey || '_UNKNOWN') + '|' + (it.phase || 'Architecture');
      (gmap[k] || (gmap[k] = { starts: [], ends: [] }));
      gmap[k].starts.push(it.s); gmap[k].ends.push(it.e);
    });
    let projMin = Infinity, projMax = -Infinity;
    items.forEach(it => { if (it.s < projMin) projMin = it.s; if (it.e > projMax) projMax = it.e; });
    const projSpan = projMax - projMin;
    const bars = Object.keys(gmap).map(k => {
      const g = gmap[k], n = g.starts.length;
      ...
      // drop the ONE element that most shortens the hull: either the earliest start or the latest
      // end, whichever removal helps more (n>=2; n==1 has nothing to drop)
      let best = rawSpan;
      if (n >= 2) {
        best = Math.min(ee[n - 1] - ss[1], ee[n - 2] - ss[0]);
        if (best < 0) best = 0;
      }
      return { k, n, rawSpan, trimSpan, oneOutSpan: best,
               rawFrac: projSpan ? rawSpan / projSpan : 0,
               trimFrac: projSpan ? trimSpan / projSpan : 0,
               oneOutFrac: projSpan ? best / projSpan : 0,
               owedToOne: rawSpan > 0 ? (rawSpan - best) / rawSpan : 0 };
    });
    const pctOver = (arr, f, key) => arr.filter(b => b[key] > f).length;
    const med = a => { const x = a.slice().sort((p, q) => p - q); return x.length ? x[Math.floor(x.length / 2)] : 0; };
    console.log('§S43_HULL ' + bld + ' bars=' + bars.length + ' projectDays=' + (projSpan / 86400000).toFixed(1) +
      ' medianBarN=' + med(bars.map(b => b.n)) +
      ' RAW>50%=' + pctOver(bars, 0.5, 'rawFrac') + ' RAW>80%=' + pctOver(bars, 0.8, 'rawFrac') +
      ' TRIMMED>50%=' + pctOver(bars, 0.5, 'trimFrac') + ' TRIMMED>80%=' + pctOver(bars, 0.8, 'trimFrac') +
      ' medianRawFrac=' + (100 * med(bars.map(b => b.rawFrac))).toFixed(1) + '%' +
      ' medianTrimFrac=' + (100 * med(bars.map(b => b.trimFrac))).toFixed(1) + '%');
    console.log('§S43_ONEOUT ' + bld + ' medianShareOfHullOwedToOneElement=' +
      (100 * med(bars.map(b => b.owedToOne))).toFixed(1) + '%' +
      ' barsWhereOneElementOwns>50%=' + bars.filter(b => b.owedToOne > 0.5).length + '/' + bars.length +
      ' RAW>50%afterDroppingOne=' + pctOver(bars, 0.5, 'oneOutFrac') +
      ' (was ' + pctOver(bars, 0.5, 'rawFrac') + ')' +
      ' trimHidesMedian=' + (100 * med(bars.map(b => b.rawSpan > 0 ? (b.rawSpan - b.trimSpan) / b.rawSpan : 0))).toFixed(1) + '%');
    bars.sort((a, b) => b.rawFrac - a.rawFrac).slice(0, 3).forEach(b =>
      console.log('§S43_WORST ' + bld + ' bar="' + b.k + '" n=' + b.n +
        ' rawFrac=' + (100 * b.rawFrac).toFixed(1) + '% trimFrac=' + (100 * b.trimFrac).toFixed(1) +
        '% oneOutFrac=' + (100 * b.oneOutFrac).toFixed(1) + '% owedToOne=' + (100 * b.owedToOne).toFixed(1) + '%'));
