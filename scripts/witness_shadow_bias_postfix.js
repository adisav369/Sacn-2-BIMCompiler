...
(async () => {
  ...
  const result = await page.evaluate(() => {
    ...
    function runAtElevation(tNorm) {
      ...
      // POST-FIX: 'before' = the OLD hardcoded -0.0005, 'after' = whatever the shipped code now
      // computes. The darkened set is therefore exactly the shadow the fix restores.
      const range = cam.far - cam.near;
      const biasLive = sh.bias;
      sh.bias = -0.0005;
      A.renderer.shadowMap.needsUpdate = true; const before = grab();
      sh.bias = biasLive;
      A.renderer.shadowMap.needsUpdate = true; const after = grab();
      const biasBefore = -0.0005, worldBefore = 0.0005 * range;
      const biasAfter = biasLive, TARGET_WORLD_BIAS_M = Math.abs(biasLive) * range;
      ...
      // acne check: a real cast shadow is CONTIGUOUS; shadow acne is high-frequency speckle. Mark
      // the darkened pixels, then count how many have fewer than 2 darkened 4-neighbours.
      const mark = new Uint8Array(total);
      for (let i = 0, p4 = 0; p4 < before.length; p4 += 4, i++) if (lum(after, p4) - lum(before, p4) < -6) mark[i] = 1;
      let isolated = 0, marked = 0;
      for (let y = 1; y < H - 1; y++) for (let x = 1; x < W - 1; x++) {
        const i = y * W + x; if (!mark[i]) continue; marked++;
        const nb = mark[i - 1] + mark[i + 1] + mark[i - W] + mark[i + W];
        if (nb < 2) isolated++;
      }
      const isolatedPct = marked ? +(100 * isolated / marked).toFixed(2) : 0;
      return {
        isolatedDarkenedPct: isolatedPct, markedInterior: marked,
  ...
  console.log('\n=== POST-FIX: old -0.0005 vs shipped computed bias ===');
  result.runs.forEach(r => console.log('elev=' + r.elevationDeg + 'deg  darkened=' + r.pixelsDarkened +
    ...
    '  brightened=' + r.pixelsBrightened + '  worldBiasNow=' + r.worldBiasAfterM.toFixed(3) + 'm' +
    '  isolatedDarkPx=' + r.isolatedDarkenedPct + '% (low = contiguous shadow, high = acne speckle)'));
  await browser.close();
