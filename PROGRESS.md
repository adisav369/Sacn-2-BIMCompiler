...
**Migration targets (initial two):** iDempiere migrator DONE+witnessed (`migrate_pg_to_sqlite.js`+`compile_rules.js`+`diff_oracle.js`+`verify_migration.js`). **Odoo source STOOD UP** — Docker `odoo`+`odoo-db` (`odoodemo`, sale/stock/account+demo), isolated from the iDempiere `postgres`. **Step-0 finding (`build/erp/odoo_fold.log`):** demo ships NO connected O2C chain — SO(sale=19)/picking(done=0)/invoice(posted=4,unpaid,no origin) all disconnected → oracle needs an RPC drive to completion then freeze static. `odoo_adapter.js` NOT written (correctly — needs the oracle first). Falsifier spec `prompts/ODOO_FOLD_POC.md`.
