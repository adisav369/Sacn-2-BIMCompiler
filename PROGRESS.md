...
## ERP write-path engine lane (`build/erp/kernel_ops.js` = source of truth; mirrors sync FROM it)
- **§I-K op-group atomicity + §I-J rate-as-op-input + BigDecimal + §I-L money-fold** — DONE, on PR #8 (`feat/erp-write-path-ik-ij` → `full`). Witnesses GREEN: poc_opgroup/poc_crud_group/poc_rate_input/poc_money_fold.
- **§I-D period-close signed checkpoint — DONE 2026-06-04, kernel v9** (additive: `kernel_checkpoints` + `closePeriod` + `latestCheckpoint` + opt-in `verifyChain({fromCheckpoint})`; default verify UNCHANGED). Witness `scripts/poc_checkpoint.js` → `§CHECKPOINT PASS` (9/9). **MEASURED I-D win:** full verify grows 24→93ms over 4 periods, bounded stays FLAT ~23ms (4× @period4). All 5 guardrail witnesses GREEN. Spec/log: `prompts/ENGINE_FULL_ERP_ISSUES.md §I-D-CKPT` + `build/erp/poc_checkpoint.log`.
- **⚠ MIRROR DRIFT (open, pre-existing):** `build/erp` kernel = v9 but env mirrors stale — `site`=v6, `docs`=v6, `deploy/dev`=v4. Mirrors were never published after I-K/I-J. Reconciliation = a full per-env publish (GO-gated). The deployed/served ERP POC does NOT yet carry the v8/v9 write-path work.
- **Open browser-lane (not forgotten):** FORK 0 write seam (JOINT re-freeze, ENGINE_CONTRACT §6.1, needs host lane) · checkpoint-anchored kernel *replay* bound (follow-on to §I-D) · cold-row eviction (compact() lane) · UI/dictated backlog (`prompts/FRONTEND_LANE_MASTER.md §OUTSTANDING`). Install/oracle lane (deferred, decided): I-A/I-B-hard/I-C/I-G/I-H.
