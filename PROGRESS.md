...
**S163 DONE (2026-04-08):** §F3 schedule/field-row overlap fixed — root cause: `row_h` not subtracting schedule height, so schedule rows bled into field rows zone. Fix: pre-compute `sch_total_h = sch_hdr_h + (1+n_rows)*sch_row_h` before `row_h` calculation. SH_FLOOR: 0 FAIL, **0 WARN** (was 3 WARN T01). §G MEP generation: `DX_2D.json` M-01 promoted GAP→STUB; dispatch updated to include STUB. DX_PLUMBING generated (10 terminals: 4 WC + 2 Sink + 2 Shower + 2 Bath). All 19 DXF files: 0 FAIL. Conformity 8/8 PASS. Remaining: §E (DX multi-storey), §F1/F2 (duplex window/room overlaps — inherent geometry), §H (dim terminator). See [prompts/S159](prompts/S159_dx_panel_legend_mep_spec.md).
