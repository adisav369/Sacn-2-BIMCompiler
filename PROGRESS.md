...
**GLASSBOWL — engine-as-data explorer (2026-05-30): LIVE read-only MVP + interactive.** (spec: `docs/GLASSBOWL.md`; Phase 2 spec: `docs/GLASSBOWL_DOSSIER.md`)
  ...
  - **NEXT (new session, spec ready `docs/GLASSBOWL_DOSSIER.md`):** Phase 2a LIFECYCLE CHAIN view first (W-LIFECYCLE — trace real record's order→ship→invoice→pay→allocate chain; oracle = poc_longtail SO 101) + sql.js data bundle; then floating card (left-click) replaces fixed panel; then right-click DOSSIER (data+rules+workflow+access+columns+accounting in one place). Editing is T3-gated (LAST).
