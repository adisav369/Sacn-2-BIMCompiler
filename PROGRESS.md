...
**4 falsifier POC prompts LEFT for a NEW session** (`prompts/*.md`, gitignored, spec-first/witness-led, reuse poc_chain/poc_distributed/poc_sign/diff_oracle, NO deploy, HANDS-OFF build/erp): **SHOWSTOPPER_POC** (checkpoint/atomicity/OLTP — self-contained, RUN FIRST) · **ODOO_FOLD_POC** (migration falsifier, needs Odoo demo dump) · **SAP_FOLD_POC** (asymptote; ACDOCA=fold, VBFA=spine; gated on IDES oracle) · **EMAIL_DR_POC** (inbox-as-log disaster matrix + KEY-recovery residual). Each is a FALSIFIER — "thesis bounded"/"needs verb X" is a valid logged result. (Spec only; not yet executed.)
