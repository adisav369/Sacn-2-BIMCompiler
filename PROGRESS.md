...
**GLASSBOWL — engine-as-data explorer (2026-05-30): LIVE read-only MVP + interactive.** (spec: `docs/GLASSBOWL.md`; Phase 2 spec: `docs/GLASSBOWL_DOSSIER.md`)
  ...
  - **Phase 2efg BUILT + PROVEN, NOT YET DEPLOYED (2026-05-30): 80/80 §GLASSBOWL-WIRING PASS (62 prior + 18 new), all pure-read/additive/0-hand-authored; gen-side §SWIPE-PICKER/§AUDIO/§QR-INPUT wired=Y, hand=0; §GLASSBOWL/§LIFECYCLE/§ORBIT still PASS.** (spec: `docs/GLASSBOWL_DOSSIER.md §Phase 2bcd` Tasks 4/5/6) — awaiting explicit go to deploy (copy `build/erp/{glassbowl.html}` → `docs/` + commit `full` + `mkdocs gh-deploy --force`; the .db bundle is byte-identical, lifecycle tables unchanged).
