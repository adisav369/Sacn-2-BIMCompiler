...
- **A-2 AD_Val_Rule interpreter ✅ (first ⛔→🟡 this arc)** — `build/erp/ad_valrule.js`: reads `ad_val_rule.code`, substitutes `@token@` from ctx, applies the where-clause as a real filter + membership test (the seam ❷ VALIDATE verb, sibling of ad_evaluator). `scripts/poc_valrule.js`→`build/erp/poc_valrule.log` (exit 0): `§VALRULE_COVERAGE total=332 static=114 token=213` (327 interpretable), 4 static filters correct, token rule 211 `@AD_Table_ID@→318`→6, `§FALSIFIER` paid invoice excluded from NotPaid. AD_Rule (4 SQL ruletype-Q Fact_Reconciliation rules, NOT Groovy) = n/a-in-seed.
- **A-3 Callout dispatch spine ✅ (⛔→🟡)** — `build/erp/ad_callout.js`: reads `ad_column.callout` (`;`-split `class.method`), resolves→JS registry, fires on field change → DERIVED sibling values (the seam ❷ DERIVE verb). `scripts/poc_callout.js`→`build/erp/poc_callout.log` (exit 0): `§CALLOUT_COVERAGE cols=284 registered=6 colsDispatched=18`; real C_OrderLine 124 derives LineNetAmt 215.90 / PriceActual 21.59 / PriceList 22.73 (== stored); `§FALSIFIER` unregistered→absent, corrupted qty→derived≠stored. 139 atoms named-deferred.
- **A-4 Model-validator timing hooks ✅ (dropped TWO ⛔)** — `build/erp/ad_modelval.js`: `fireHooks(timing,...)` over 11 BEFORE/AFTER timings (port of fireModelChange/fireDocValidate, first error aborts); GATES the action, never posts (seam A-4). `scripts/poc_modelval.js`→`build/erp/poc_modelval.log` (exit 0): `§MODELVAL_HOOK timing=BEFORE_COMPLETE table=C_Order fired=2 ok=true`; `§FALSIFIER` qty≤0 line + 0-line order BLOCKED; un-hooked→explicit no-op. 3 Java validators + 322 beforeSave/afterSave named-deferred.
- **A-5 C_DocType FSM ✅ (⛔→🟡)** — `build/erp/ad_docfsm.js`: port of DocumentEngine.getValidActions/processIt; `legalActions(db,doctypeId,status)` (per-doctype) + `transition(action,status)→toStatus`, pure status machine. `scripts/poc_docfsm.js`→`build/erp/poc_docfsm.log` (exit 0): reaches 11 of 12 statuses (was 2), Standard Order 132 dispatches full reversal family {CL,RC,RA,RE,VO,PO}, `§FALSIFIER` illegal transitions rejected.
- **A-6 Workflow engine ✅ (dropped TWO ⛔) — TRACK A COMPLETE** — `build/erp/ad_workflow.js`: `walk(db,wfId,{ctx})` reads AD_WF_* graph, walks node→next from start, creates an activity per node, routes splits (std-user-next / approval decision); seam ❶ honored (no GL knowledge). `scripts/poc_wf.js`→`build/erp/poc_wf.log` (exit 0): `§WF_COVERAGE workflows=58 nodes=262 nexts=207`, full walk 50000>50002>50001, split 200033 std→200034 vs approval→200036, `§FALSIFIER` leaf terminates + off-graph falls back.
- **Option-(a) minor SQL/FK surfaces ✅ (4 more ⛔→🟡)** — `build/erp/ad_tabquery.js` (W-TABQUERY: AD_Tab.WhereClause applied as row filter + OrderByClause as sort) + `build/erp/ad_reference.js` (W-REFERENCE: AD_Ref_Table FK membership query + AD_Column.ValueFormat mask). `scripts/poc_tabquery.js`/`poc_reference.js`→`build/erp/poc_{tabquery,reference}.log` (exit 0): `§TAB_FILTER tab=186 rows=4`, PO excluded from SO tab; `§FK_CHECK id=1 exists=true`, 999999 rejected; `§VFORMAT mask=L a→A`, digit rejected.
- **Matrix:** `docs/ERP_COVERAGE_MATRIX.md` now **0✅ / 37🟡 / 3⛔** (was 14⛔ at arc start; dropped 11). **Every interpretable behavioural surface with seed data is 🟡 — zero real interpreter gaps.** The 3⛔ are ALL n/a-in-seed: AD_Rule (4 SQL ruletype-Q Fact_Reconciliation rules, NOT Groovy — fact_acct/fact_reconciliation empty + Postgres SQL) + 2 empty *_Access. **NEXT (user's call):** Track B §H-7+ substrate · commit · or resume parked UI bridge (🟡→✅). UI bridge PARKED; history timeline untouched.
...
- **AD-gen installer STRUCTURE** (2026-06-03, `full`, NOT deployed): `scripts/gen_ad.js` + `scripts/error_report.js` — dictionary→AD seed (DB+Excel providers, majority-infer, ErrorReport rubbish-trap, role-id entity/identifier/amounts/key, line→header FK nest). Spec `docs/AD_GEN_FROM_DICTIONARY_SPEC.md §3/§10`. Witnesses `build/erp/ad_gen*.log` (`§AD-GEN`/`-REPORT`/`-ROLE`/`-RUBBISH`). HELD: `§AD-RENDER` (T4) + deploy.

## Lens-family doctrine — published docs (2026-06-03, thinking-hats session; DOCS only, no code/deploy)
The serverless/no-server ERP framing consolidated into a doc family, extracted from RED1's iDempiere Unicenta POS + plugins. Hub `docs/LensFamily.md` → `docs/{POSLens,WMSLens,SocialPlatformLens,CreditLedgerLens,WorkforceLens,GuaranteedChannels}.md`, all tracing to `docs/DistributedERP.md` (§3 worked-example link added). Core: one source act → fold (GIGO solved); sacred transaction (provenance unbroken — key the master, never the sale; no free numbers); no central control / use what is guaranteed (email/SQLite/browser/native-intents; server deletes itself); ride killer apps; **horizontal = iDempiere AD already inside** (`verify.log` PASS), **lenses = cheap verticals** (view/provider/stub, fold-not-fork); horizontal consolidates the long tail (no silos, BI = the fold, period-close bounds). Ideas mined (Khatabook/Glide/Streak/local-first/M-Pesa): credit-ledger = AR fold (`SO_CreditLimit`/`C_PaymentTerm`, integrated into POS, bankable-history value path), Drive-encrypted channel, USSD feature-phone tier. Impl prompt `prompts/POS_LENS_SESSION.md` (gated on write-path D/E).
**WATCHDOG (2026-06-03, `prompts/FUNDAMENTALS_WATCHDOG.md` → `build/erp/fundamentals_watchdog.log`):** committed arc — **11 PASS · 1 DRIFT · 0 BREAK**; drift = B8 signed chain not yet over the LIVE write log (the named I-4 schema mismatch — owned by engine lane, gate before write-path deploy). Re-run each dev cycle / after lanes commit.
...
**Front-door pill UX ✅ FINISHED LIVE (2026-06-09, bim-ootb sw v610, PR #214):** `prompts/FRONT_DOOR_PILL_FINISH.md` worked to zero — that DONE ledger cites every spec/js/witness (§P4 = Install/Migrate pill-only · true strip-peek · context-gated ShowMe via additive `help_overlay.js setOps`, no fork; spec addendum `prompts/READSHOWME_DYNAMIC_SPEC.md §P4`).

## FRONTEND lane — Item C Accts-Posted lens DEPLOYED LIVE (2026-06-03, PR #94, sw v565)

Master `prompts/FRONTEND_LANE_MASTER.md` §2 Item C. Spec+DONE: `prompts/ACCTS_POSTED_PANEL.md`; orchestration `docs/DATA_ACQUISITION_ORCHESTRATION.md`. `bim-ootb/erp/accts_posted.js` (`buildCtx`/`buildPostedVM`/`mount` desktop + `mountAccordion`/`pillControls`/`bindBack` mobile, consumes `window.ERPPostings`, no fork) + `tests/poc_accts_posted.{js,log}`. **✅ ALL PASS** — §POSTED-READ/-GATE (DOM zero-leak)/-COVERAGE/-CTX/-MOBILE (accordion, pills `[install,share,verify]`). One fold, two lens renders. **DEPLOYED:** PR #94 merged to bim-ootb `main` (CI green), **pill-ALONGSIDE** the classic header (`[[project_pill_alongside]]` — augment-then-replace, borrow Time-Machine scrubber for undo/redo); read-only until install → honest "install local data first". Fetch-back-verified live (sw v565, scripts 200, pill present). §POSTED-LIVE log on open. Bundle also shipped the engine UMD copies (`erp_kernel.js`/`erp_seam.js`) + STEP-0 write-path spike.

**Design decisions LOCKED (this session, recorded so they don't drift):** mobile = accordion lens reusing `ad_ui.js .acc` + double-tap drill + phone-back; Install/Migrate on mobile = **delegate-to-machine + mirror** (phone is glass); QR = **pointer+hash to the operator's own host**, pure-data payload (code resident) — `DATA_ACQUISITION_ORCHESTRATION §0.1`; phone **range-streams "all" on read** (no pre-shard). 3 UI pillars (`[[project_lens_family]]`): Kanban dashboard (Odoo) · iDempiere desktop (shard-login) · graph pills. **POC SHIPPED localhost (2026-06-03, dev :9090, sw v568, NOT gh-pages):** pill rail (Lucide icons, no emoji, glassbowl/gravity removed) + RED-PILL 3-state toggle + empty-start dashboard (tiles=6 real) + Graph/Kanban switchable views + mobile cards + Migrate→ShowMe + Install→QR-pair + **POC-DEMO signed writes** (drag→SET_STATUS, New/Save/Delete via `kernel_ops.js`). State + GAP LEDGER → `prompts/FRONTEND_LANE_MASTER.md ▶POC SHIPPED`. **Engine write-gates (new-session discussion):** `prompts/ENGINE_FULL_ERP_ISSUES.md` (resolve I-A…I-I → wire each write). Named gaps: projection persistence (op-log yes, IDB no — `APP.DB_URL` unset), T1/T2 streaming, Attach, real posting. I-4 decided (POC): deployed signed `kernel_ops.js`.
...
**Migration-Solvent Campaign** — detail: [HolyGrail §migration](docs/HolyGrail.md) · resume cards `prompts/{MIGRATION_CAMPAIGN_RESUME,SPECS_AND_STRATEGY_RESUME}.md` · logs `build/erp/{odoo,sap}_fold.log`. Status:
- iDempiere migrator DONE (`migrate_pg_to_sqlite`+`compile_rules`+`diff_oracle`+`verify_migration`).
- **Odoo CLOSED** — 6 chains fold, `newVerbs=[]`, 1 engine change (f8 partial-qty matcher); f2 partial-pay + f1 account-derivation PASS; 11 runners 0🔴. `scripts/odoo_adapter.js` · `poc_odoo_fold*.js` · `build/erp/odoo_oracle*.json`.
- **SAP skeleton + flight 2nd-source GATED** (`§SAP-FOLD BLOCKED` — no oracle = non-invent) — `scripts/sap_adapter.js` · `poc_sap_fold{,_flight}.js` · `build/erp/sap_oracle*.template.json` · spec `prompts/SAP_FOLD_POC.md`. Needs a real SD+FI export to fold the FI half.
- **NEXT (render → installer):** `docs/AD_GEN_FROM_DICTIONARY_SPEC.md` (AD-gen from dictionary; gated `§AD-GEN`/`§AD-RENDER` proof) → `docs/ERPMaker.md` installer (gated on that proof).
