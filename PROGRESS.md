...
### Pristine BOM.db — IN PROGRESS (spec done, implementation next session)

**Governing spec:** `docs/DATA_MODEL.md` §1 — rewritten 2026-03-10 as creation spec.
**Status:** DATA_MODEL.md committed. BOM.db still corrupted (rename → BOMbad.db next session).

**Next session — implement `generate_extracted_bom()` in `scripts/RosettaStoneToBOM.py`:**
1. `mv library/BOM.db library/BOMbad.db`
2. Expand RosettaStoneToBOM.py: schema creation + reference data + extracted BOMs
3. Key formula: `dx = element_centroid - building_LFD` (centroid offset, NOT LFD corner)
4. `allocated_*_mm = element AABB × 1000` (per-instance dims, not M_Product fallback)
5. BUILDING origin = building LFD corner (world coords). Floor origins = (0,0,0).
6. Floor links: MAKE lines at dx=0. Element lines: BUY with centroid offsets.
7. Integrity hash in ad_sysconfig.

**M_Product + C_DocType:** Must be generated by script too (FOSS reproducible).
Derive M_Product from I_Element_Extraction. C_DocType as Python data literals.

See `docs/DATA_MODEL.md` §1 for full algorithm and `.claude/plans/tidy-squishing-giraffe.md` for implementation plan.
