...
import com.bim.ormsandbox.po.X_W_Verb_Node;
import com.bim.ormsandbox.po.X_W_Verb_NodeProduct;
...
/**
 * Witness tests for W_Verb_Node + W_Verb_NodeProduct PO classes.
 *
 * <p>Uses in-memory SQLite — no BOM.db dependency.
 */
@DisplayName("W_Verb_Node — Verb Invocations PO")
class W_Verb_NodeTest {
    ...
    @Test
    @DisplayName("W-VN-1: Insert and read W_Verb_Node via PO")
    void insertAndReadNode() throws SQLException {
        ...
        int id = node.getW_Verb_Node_ID();
        assertTrue(id > 0, "auto-increment PK must be assigned");

        M_W_Verb_Node loaded = M_W_Verb_Node.get(conn, id);
    ...
    @Test
    @DisplayName("W-VN-2: getByOrder returns nodes sorted by SeqNo")
    ...
    @Test
    @DisplayName("W-VN-3: Insert and read W_Verb_NodeProduct parameters")
    ...
    @Test
    @DisplayName("W-VN-4: DocStatus CHECK constraint enforced")
    ...
    @Test
    @DisplayName("W-VN-5: UNIQUE(W_Verb_Node_ID, Name) enforced on NodeProduct")
    void uniqueParamName() throws SQLException {
        ...
        M_W_Verb_NodeProduct dup = new M_W_Verb_NodeProduct(conn);
        dup.setW_Verb_Node_ID(nodeId);
    ...
    private int insertNode(String orderId, int seqNo, String name, String desc) throws SQLException {
        ...
        node.save();
        return node.getW_Verb_Node_ID();
    }

    private void insertParam(int nodeId, String name, String value, String type) throws SQLException {
        M_W_Verb_NodeProduct p = new M_W_Verb_NodeProduct(conn);
        p.setW_Verb_Node_ID(nodeId);
        p.setName(name);
        p.setValue(value);
        p.setValueType(type);
