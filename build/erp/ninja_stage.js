...
(function (global) {
  ...
  function extractModel(db, AD_Window_ID) {
    ...
    var modelTables = tables.map(function (tbl) {
      ...
      // master FK col is generated by stageModels ONLY when TabLevel > 0
      // — identify it as the first non-standard *_ID TableDir col in that case
      var masterCol = null;
      var masterFkSeen = false;
      ...
      cols.forEach(function (c) {
        ...
        // master FK: only present when TabLevel > 0; first *_ID TableDir col that appears before user cols
        if (!masterFkSeen && tabLevel > 0 && nm.slice(-3)==='_ID' &&
            (c.AD_Reference_ID===DT_TABLEDIR||c.AD_Reference_ID===DT_TABLE)) {
          masterCol = nm.slice(0,-3);               // strip _ID → master table name
          masterFkSeen = true;
          return;                                   // skip the generated FK col
        }
