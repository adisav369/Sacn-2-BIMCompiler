...
(function () {
  ...
  function placeMeasured(disc, bdb, opts) {
    ...
    // §TE-ARC-DATUM-FIX (RESUME_DISC_WALKER_ENVELOPE_BOUND.md §TE-ARC-DATUM, 2026-07-10): the baked
    // z_datum_offset targets ONE frame (the mining extraction's) — the shipped Terminal_ARC.db sits
    // 14.66 m below it and every band starved. With rule_frame_ref (per-class mean_z refs in the BAND
    // frame, stamped from measured data), MEASURE the offset from the walked substrate itself:
    // median over shared non-generatable classes of (walked class mean z − ref mean z). Same-frame
    // substrates reproduce the legacy offset exactly; any pure-translation frame reconciles. <3
    // shared classes → honest fallback to the legacy constant, logged.
