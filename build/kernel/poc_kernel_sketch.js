// W-SKETCH-SOLVE harness — prompts/BONSAI_KERNEL_RESEARCH.md §5b Item 1c.
...
const server = http.createServer((q, r) => {
  let p = decodeURIComponent(q.url.split('?')[0]); if (p === '/') p = '/build/kernel/spike_sketch.html';
...
(async () => {
  ...
  const logs = [];
  pg.on('console', m => { const t = m.text(); if (/^§SKETCH/.test(t)) { logs.push(t); console.log('  ' + t); } });
  pg.on('pageerror', e => console.log('  ERR ' + String(e).slice(0, 300)));
  await pg.goto(`http://localhost:${port}/build/kernel/spike_sketch.html`, { waitUntil: 'load', timeout: 60000 });
  await pg.waitForFunction('window.__done === true', { timeout: 90000 }).catch(() => console.log('  ✗ timeout waiting for __done'));
  await br.close(); server.close();
  const verdict = logs.find(t => t.startsWith('§SKETCH VERDICT'));
  const pass = verdict && verdict.includes('PASS');
  console.log('── W-SKETCH-SOLVE ' + (pass ? 'PASS' : 'FAIL') + ' ──');
