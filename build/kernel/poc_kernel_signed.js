// W-KERNEL-SIGNED harness — prompts/BONSAI_KERNEL_RESEARCH.md §5 Item 1 (GO/NO-GO).
...
const ROOT = REPO;   // serve repo root so /node_modules/occt-wasm/... and /build/kernel/spike_signed.html resolve
const MIME = { '.html':'text/html', '.js':'text/javascript', '.mjs':'text/javascript',
  '.wasm':'application/wasm', '.json':'application/json', '.css':'text/css' };
const server = http.createServer((q, r) => {
  let p = decodeURIComponent(q.url.split('?')[0]); if (p === '/') p = '/build/kernel/spike_signed.html';
...
(async () => {
  ...
  const logs = [];
  pg.on('console', m => { const t = m.text(); if (/^§KERNEL-SIGNED/.test(t)) { logs.push(t); console.log('  ' + t); } });
  pg.on('pageerror', e => console.log('  ERR ' + String(e).slice(0, 300)));
  await pg.goto(`http://localhost:${port}/build/kernel/spike_signed.html`, { waitUntil: 'load', timeout: 60000 });
  await pg.waitForFunction('window.__done === true', { timeout: 90000 }).catch(() => console.log('  ✗ timeout waiting for __done'));
  await br.close(); server.close();
  const verdict = logs.find(t => t.startsWith('§KERNEL-SIGNED VERDICT'));
  const pass = verdict && verdict.includes('PASS');
  console.log('── W-KERNEL-SIGNED ' + (pass ? 'PASS' : 'FAIL') + ' ──');
