...
| 15 | Building parts taxonomy (STAIRWAY/LIFT_SHAFT/PLANT_ROOM) | 9 | Real multi-building ground truth for STAIRWAY (5 buildings) + LIFT_SHAFT (4 buildings); top-down/bottom-up checklist, W-BUILDING-PARTS 13/13 PASS; BOTH VISION-LOCK sentence-5 UI halves wired + MANAGER-verified on real data — Find panel (bim-ootb `d04ddd5`) and Modeller Outliner (bim-ootb `f10c5295`, caught+fixed a real leaf-id click-path bug in the process) | PLANT_ROOM is Terminal-only (n=1); STORAGE/AIR_LIGHT_WELL explicitly refused (zero real evidence anywhere); `required_spaces`-style hard gating not added (deliberately advisory-only) |
