...
| 15 | Building parts taxonomy (STAIRWAY/LIFT_SHAFT/PLANT_ROOM) | 8 | Real multi-building ground truth for STAIRWAY (5 buildings, Hospital=60 strongest) + LIFT_SHAFT (4 buildings); top-down/bottom-up checklist walks the SAME config either direction; W-BUILDING-PARTS 13/13 PASS; Find-panel Parts axis wired + MANAGER-verified on real Duplex/SampleCastle data (bim-ootb `d04ddd5`, local-only) | PLANT_ROOM is Terminal-only (n=1); STORAGE/AIR_LIGHT_WELL explicitly refused (zero real evidence anywhere); Modeller Outliner half of VISION-LOCK sentence 5 still not wired |
