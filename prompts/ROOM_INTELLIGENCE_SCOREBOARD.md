...
| 15 | Building parts taxonomy (STAIRWAY/LIFT_SHAFT/PLANT_ROOM) | 9 | Real multi-building ground truth for STAIRWAY (5 buildings) + LIFT_SHAFT (4 buildings); top-down/bottom-up checklist, W-BUILDING-PARTS 13/13 PASS; BOTH VISION-LOCK sentence-5 UI halves wired + MANAGER-verified on real data — Find panel (bim-ootb `d04ddd5`) and Modeller Outliner (bim-ootb `f10c5295`, caught+fixed a real leaf-id click-path bug in the process) | PLANT_ROOM is Terminal-only (n=1); STORAGE/AIR_LIGHT_WELL explicitly refused (zero real evidence anywhere); `required_spaces`-style hard gating not added (deliberately advisory-only). **NEW 2026-07-11, found+confirmed during `VIEWER_FIND_PANEL_PARTS_VERIFICATION.md`, not fixed (out of that task's scope):** `PLANT_KEYWORDS`'s bare `LIKE '%vent%'` false-positives on any name containing "vent" as a substring (confirmed live on Duplex — 4/6 "Plant Room" hits are actually `Backflow Preventer` valves, only 2 are real MEP plant); separately, the live Find-panel Parts axis has no class gate at all (unlike `checklistReport()`, which is `complex`-only for Plant Room), so a residential building can surface a technically-real but semantically-wrong Plant Room count to a user. Two small, named, un-dispatched fixes — word-boundary match + class-gate the live query — left for whoever next touches this axis.
