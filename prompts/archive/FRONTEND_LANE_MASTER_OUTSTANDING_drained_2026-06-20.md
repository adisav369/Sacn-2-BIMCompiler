# ARCHIVED 2026-06-20 — FRONTEND_LANE_MASTER §OUTSTANDING (fully drained)
# Retired from prompts/FRONTEND_LANE_MASTER.md by user decree ("retire that fully drained so it wont arise again").
# Every item below is ✅ DONE (witness) or ⛔ BLOCKED. Kept verbatim for provenance/search only — NOT a live backlog.
# The three still-blocked items were lifted forward into the master file under §OUTSTANDING — RETIRED.
...
> **▶ ✅ COMPLETE — iDempiere-FAITHFUL IN-PLACE CRUD. Card: `prompts/CRUD_INPLACE_EDIT_SESSION.md`. P1–P5 ✅ DONE/LIVE
> 2026-06-17.** (P5 = sw v709, crud_overlay?v=19, W-INPLACE-UNDO-LIVE 10/10: Ignore↔timeline distinction witnessed +
> the phantom-draft-pip FIXED — an inline form buffers the draft against the POST-render `_inlineBaseline`, so an
> untouched open raises no spurious unsaved pip.) Next lane = ERP CRITIC spine (GRAND_LANE_STRATEGY P3 surpass axes). P2 = inline EDIT (PR #353, sw v704, crud_overlay?v=16, W-INPLACE-EDIT-LIVE 12/12): form view editable
> in place, NO #crudForm modal, NO ✎ Edit button (retired, supersedes #351 W-RING-LEAK); Save/Ignore dirty-gated,
> Ignore=dataIgnore. P3 = inline NEW/COPY/SAVE&NEW/DELETE (PR #354, sw v705, crud_overlay?v=17, W-INPLACE-NEWDEL-LIVE
> 14/14): full verb bar AD-folded; createInline/copyInline; inline delete-confirm; untouched-New auto-discards on nav;
> create-form dirty-from-start. P4 = GRID INLINE EDIT (sw v706, crud_overlay?v=18, W-INPLACE-GRID-LIVE 11/11): click a
> grid data cell → an inline WEditor (`editCell`, reuses fieldInput/populateRefs) → Enter/blur commits ONE signed
> CRUD_UPDATE row-wise (GridView/GridTabRowRenderer parity); Escape/unchanged reverts; a read-only cell falls through
> to opening the record form (not a dead click); docstatus cell rides DOC_ACTION via splitStatusChange. Fixed a latent
> listTip overlay bug (an UPDATE keyed lowercase via f.col was invisible on the SELECT* original-case grid row → now
> updates the existing-cased key). Same signed engine, only the MOUNT moved (fhost). 14 regressions GREEN.
> **NEXT = P5 Ignore↔timeline reconcile (W-INPLACE-UNDO-LIVE) + phantom-draft-pip verify.** Resume:
> `prompts/RESUME_INPLACE_P4.md`. (Original gap/plan below, kept for context.)
> THE GAP: our edit UX is inverted vs iDempiere — read-only form → a **✎ Edit button** → a **MODAL `#crudForm` popup**.
> User: "edit happens in the panel form itself or even grid row-wise — there is NO popup, NO Edit button; only
> Save/Delete; undo = the history timeline, but unsure if consistent with iDempiere's normal undo." PLAN FROM SOURCE
> (we HAVE it): iDempiere ZK `org.adempiere.ui.zk/.../adwindow/` — `ADWindowToolbar` verbs = New/Copy/Delete/Save/
> Save&New/**Ignore(=undo, Alt+Z)**/Refresh/GridToggle, **NO Edit button**; `AbstractADWindowContent` dataIgnore()/
> needSave()/dataStatusChanged (dirty gating); `GridView`+`GridTabRowRenderer` = inline-editable cells. So the form
> AND grid are always-editable; Save/Ignore gate on dirty; Ignore reverts uncommitted edits (distinct from our
> committed-op timeline). FIX = move the MOUNT (reuse crud_overlay's SIGNED write seam: fieldInput/populateRefs/
> applyAdLogic/callout/buildOp→applyOp), render the editable fields INLINE in the form view, retire ✎ Edit + the
> modal. Phased P2 inline-edit → P3 New/Copy/Delete → P4 grid-inline → P5 undo-reconcile; witnesses W-INPLACE-*.
> NOTE: this SUPERSEDES the ✎ Edit shipped in #351 (W-RING-LEAK) — the ✎ button goes away entirely. Own focused
> session recommended (core-path refactor, full regression surface). RING-LEAK is still ✅ until this lands.

> **✅ DONE (witness) 2026-06-17 — S2B AD-FOLDED CRUD GENERALITY shipped (bim-ootb PR #348, sw v701,
> crud_overlay.js?v=15, ad_parser.js?v=24).** `CORE.foldCrudSpec` derives editability FROM the AD (type via
> authoritative AD_Reference_ID, read-only via IsView+IsReadOnly+IsUpdateable, verbs gated, context defaults
> resolved); FOLDED fallback in entryFor; host folds `_curTab()` + registers before create/update/remove.
> crud_ops.json is now an OPTIONAL override (5 doc tables keep docPolicy fan-out). Audit gaps (b) warehouse from
> AD_OrgInfo (c) DocNoSequence_ID (d) MV registry-discovery ALL closed. W-AD-FOLDED-CRUD 14/14 (headless) +
> W-AD-FOLDED-CRUD-LIVE 14/14 (C_BPartner New/Edit/Delete signed+survive reload; IsUpdateable=N display-only;
> VIEW read-only). Regressions GREEN (crud-full/create/process/form-pill/grid-batch/gridstatus/recinfo/draft/blue).
> Black Book note: detect-from-AD is general (CRUD); EXECUTE (Process) is gated to ported DOC_FAMILY (consequences
> extracted not invented) — C_Project has no Process because MProject isn't a DocAction model. NEXT = resume the
> SPINE: S3/J6 POST (prompts/S3_J6_POST_SESSION.md, GL to the cent), then ERP CRITIC P3 surpass (S4 self-edit ×5).
>
> **(superseded — kept for context) S2B — AD-FOLDED CRUD GENERALITY ("general not custom"; Janke AD vision). Card: `prompts/S2B_AD_FOLDED_CRUD_SESSION.md`.**
> WHY PROMOTED: this was spec'd but parked as a tributary, so work-to-zero never picked it up — it lost to a spine
> leg every session and stayed invisible. Now it IS the top item; drive it to ✅/⛔, don't re-park.
> SCOPE: build `foldCrudSpec(table)` — derive editability from the dictionary, NOT a hand-curated list: fields from
> `AD_Field`/`AD_Column`, read-only from `AD_Table.IsView` + `AD_Tab.IsReadOnly` + `AD_Column.IsUpdateable` + role.
> Then RETIRE the curated 5-table `crud_ops.json` allow-list (`c_order/m_inout/c_invoice/c_payment/c_allocationline`).
> WITNESS: `W-AD-FOLDED-CRUD-LIVE` — assert `C_BPartner` (and any non-curated AD table) becomes New/Edit/Delete-capable
> per its dict, with NO curated entry. Plus the 4 AUDIT gaps that ride the same "fold-from-AD" build:
> (1) crud editability derived from AD (above); (2) `_docCtx` default warehouse from `AD_OrgInfo.M_Warehouse_ID`
> (not the "lowest org wh id" heuristic); (3) `_allocDocNo` honours `C_DocType.DocNoSequence_ID` when IsDocNoControlled;
> (4) `MV_INSTALLER` discovered from the registry (not the hardcoded 4-table map).
> ⚠ CORRECTION (verified in code 2026-06-17, against a stale "Process deferred" review): the **Process/DocAction
> browser leg is ALREADY DONE + LIVE** — see §OUTSTANDING item 2 ("WIRE FULL DOCACTION SET PER AD ✅") + J5 PROCESS
> (bim-ootb #329, sw v694). `_showProcessChooser`/`buildDocActionBar`/grid gear fold the FULL FSM legal set via
> `AdDocFsm.legalActionsOrder/legalActionsFor` (NOT the `{action:"CO"}` descriptor default); engine W-DOCACTION-FULL
> 14/14 + live W-CRITIC-PROCESS asserts a non-CO action (ReActivate CO→IP). So "do Process first" is a NO-OP — the
> single open item is S2B CRUD-generality. (BIM→Project Order: Process is free for a walked DocAction doc like
> `C_Order`; `C_Project` is NOT FSM-walked; creating/editing any folded doc on the generic surface waits on S2B.)
> AFTER S2B: resume the ERP CRITIC lane P3 surpass axes (S4 self-edit ×5 / S6 lens-swap) — see `prompts/ERP_CRITIC_UX_LANE.md`.

> **✅ DONE (witness) 2026-06-17 — P3/S4 LIVE MODEL SELF-EDIT ×5 (bim-ootb PR #350, sw v702, W-CRITIC-SELFEDIT 30/30,
> erp/tests/poc_critic_selfedit_live.js).** A signed dictionary edit (AD_Field.IsDisplayed) refolds the OPEN window on
> the spot — re-read, not recompile — identically on all 5 tenants (substrate invariant, zero per-tenant code). ACT A:
> __crud.applyOp signs the CRUD_UPDATE → §AD-SELFEDIT-LIVE refold → the column VANISHES live, ×5. ACT B (Odoo): the
> served BUNDLE row stays IsDisplayed=Y (edit lives only in the op-log), reverse edit (N→Y) brings it back = re-FOLD not
> recompile. REGRESSION FIXED: the overlay:committed data-CRUD branch (J4 CREATE #331) returned for EVERY CRUD op,
> shadowing the dictionary refold (#312) — now an AD_* CRUD_UPDATE falls through to the model refold. Regressions GREEN
> (crud-full/create/process/folded-crud/form-pill). NEXT P3 axis: S6 lens-swap / S1 offline-5 / S3 blue / S5 / S7.
>
> **✅ DONE (witness) 2026-06-17 — RING LEAK ON iDempiere FIXED (bim-ootb PR #351, sw v703, W-RING-LEAK 10/10,
> erp/tests/poc_critic_ring_leak_live.js).** Re-pointed the ✎ CRUD toolbar button ("✎ Edit") to the form host-seam
> (_openEdit → __crud.update → edit FORM, ring NOT fanned); enable()/openRing()/Edit-mode toggle never called from the
> iDempiere surface → §CRUD ring view=on + §CRUD-IDMP-OPEN can no longer fire (asserted on c_bpartner + c_order). The
> ring stays Glass/Gravity-only; migrate_status_panel CRUD claim unchanged (form path is iDempiere-native). Regressions
> GREEN (form-pill/crud-full/create/folded-crud). (superseded the ⛔→OPEN note below — kept for context.)
>
> **⛔→OPEN — RING LEAK ON iDempiere (doctrine §0 violation, found live 2026-06-17 via user §-log).** GRAND_LANE
> §0 is explicit: "the visual CRUD ring is Glass/Gravity ONLY; **iDempiere NEVER opens it**; Execute = iDempiere UI
> surface, never the ring." But the legacy **`✎ CRUD` toolbar button** (idempiere.html:1088) still does
> `window.__crud.enable()` + `openRing(tn)` → `§CRUD ring … view=on` + `§CRUD-IDMP-OPEN` on the iDempiere surface.
> It predates the J4/J5 re-point onto the form and was never retired; **S2B widened it** — `_crudHas` is now
> foldable-aware, so the ring fans for `c_bpartner` and EVERY folded table, not just the curated 5. The
> doctrine-correct CRUD (form pills → `_openEdit`/`_openCreate` → host seam → edit FORM, `ring not fanned`) already
> works in parallel (`§FORM-PILL edit=edit-form … ring not fanned`). FIX (one bounded idempiere.html leg, the lane's
> to scope — re-point vs remove is a UX call): retire the ring on the iDempiere surface — re-point `✎ CRUD` (and any
> Edit-mode toggle that fans hots/ring here) to the form host-seam path; the ring stays Glass/Gravity-only. WITNESS:
> a live test asserting `✎ CRUD` on `c_bpartner` opens the FORM and `§CRUD ring … view=on` / `§CRUD-IDMP-OPEN` NEVER
> fire on iDempiere; form-pill + CRUD-full + S2B regressions stay green. NOTE: the migrate_status_panel "CRUD is
> general / New-Edit-Delete on iDempiere's own screens" claim stays TRUE (the form-pill path is iDempiere-native);
> this is UI-surface hygiene, not a capability gap. Cross-ref: S2B card §STANDING + RESUME_HERE.

> **▷ TRIBUTARY — AD_PROCESS FOLD lane (card `prompts/AD_PROCESS_FOLD_LANE.md`, written 2026-06-17).** Populate
> AD_Process handlers by demand (the "processes run on demand" half of the journey). Dispatch SPINE already LIVE
> (ad_process.js W-PROC + §AD-PROC-LIVE) — adds handlers only, KIND 1 (report-fold) / 2 (engine-verb via buildDoc,
> newVerbs=0) / 3 (registered plugin). FIRST leg = Generate Order from Project: `C_Project.GenerateTo` button →
> `AD_Process 164` (`ProjectGenOrder`) → fold project lines → signed C_Order(PO) op-group (W-PROC-GENPO). RESUMABLE
> standalone (low conflict; different idempiere.html functions from the ring-leak fix + S4). ⚠ ONE dependency:
> GenerateTo is a ref-28 BUTTON and S2B foldCrudSpec DROPS buttons → P1 invokes process 164 via the MENU/chooser
> (decoupled, default); rendering the button on the form is a small S2B follow-on (2nd leg). Tributary — slot after
> the spine reaches process-depth or as a user-prioritised insert; return to the spine after.

> **▶ DOC-PANEL / KERNEL-LOG SAVE+AUDIT BAND (dictated 2026-06-14) — 5 items, all ERP-UI, all idempiere.html.**
> **NEXT-SESSION CARD: `prompts/USER_JOURNEY_LANE.md`** (BE THE USER — walk the live experience vs the spec promise,
> rank gaps by leverage, fix to zero with best judgement; folds in remaining items 2/4/5). Items 0/1/3a/3b shipped
> live (PRs #314/#317/#320/#322; sw v689). Item 1 (draft restore) ✅ DONE PR #322; item 0 ✅ DONE PR #317.
...
> 4. ✅ DONE (witness) **GRID MULTI-SELECT + GEAR BATCH PROCESS** — SHIPPED + LIVE (Opus 2026-06-15, bim-ootb
>    PR #323, erp sw v690). idempiere.html buildGrid/_buildCards gain a checkbox column + select-all; selection
>    keyed by record PK (`_gridSel`, survives re-render, reset on reload); `#idmp-batchbar` "gear" toolbar fans
>    `_fsmCtx(tab,rec).dispatch` over the LEGAL-ACTION INTERSECTION of the selection (factored `_fsmCtx` shared
>    with buildDocActionBar — "AdDocFsm.dispatch over N rows", never a forked verb). **W-GRID-BATCH 7/7 PASS**
>    (`erp/tests/poc_grid_batch_live.js`): 2 CO invoices → [CL,RC,RA] → CL fans ok=2 → re-folds → ✕ clears, 0
>    pageerrors. Spec `prompts/GRID_BATCH_FORM_PILL_SPEC.md`. ORIGINAL DICTATION: rows checkbox-selectable → gear
>    hosts batch processing (process/complete/delete N); ring stays for quick single-row.
> 5. ✅ DONE (witness) **PROCESS (+New/Save/Print) IN PILL FOR FORM VIEW** — SHIPPED + LIVE (Opus 2026-06-15,
>    bim-ootb PR #323, erp sw v690). red-pill "just the pill" hides #idmp-toolbar → three form-view-gated pills
>    (`showWhen:"form-view"` via `window.IdmpPillFormGate`, re-eval in renderBody): **New** (`plus`→CRUD ring),
>    **Save** (`save`→open form `#cfSave`), **Process ▶** (`next`→DocAction chooser over the `_fsmCtx` legal set).
>    Print maps to the existing `reports` pill (NON-INVENT). Each binds BY ID to a real handler — no crud_overlay
>    change. **W-FORM-PILL-ACTIONS 6/6 PASS** (`erp/tests/poc_form_pill_actions_live.js`): grid→off, form→all
>    three mount, Process ▶→[CL,RC,RA], New→CRUD ring, 0 pageerrors. `idmp_pills.js?v=13` + `pills_idmp.json?v=32`.
